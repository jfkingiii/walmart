...
#Week blending
drop_dates <- c("2012-11-02", "2012-11-09", "2012-11-16", "2012-12-07", "2012-12-14", "2012-12-21", "2012-12-28",
                "2013-02-08")
...
# Store 38 adjustment  - might make trend prospective as well
ind <- which(store == 38)
wks_between <- as.integer(difftime(dt[ind], min(dt[ind]), units="weeks"))
...
sub$sales <- with(sub, sales + ifelse(MarkDown1 > 4970 & MarkDown1 <= 8810 
                                      & dept == "87" , 2238, 0))
# Gives ~4 point improvement, promising since indicated sales increase is 2428
