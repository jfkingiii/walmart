...
# Departments
# 1 Candy Food Tobacco
# 2 Health and Beauty aids
# 3 Stationary and books
# 4 Paper goods
...
# 56 Horticulture
# 71 Furniture
# 72 Electronics (biggest error)
# 74 Storage
# 77 Large appliances
# 92 Dry grocery (big error)
# 95 Direct store delivery (big error)
# 96 Liquor
# 98 Bakery
...
# Store 39 adjustment - improvement < 1
ind <- which(store == 39)
wks_between <- as.integer(difftime(dt[ind], min(dt[ind]), units="weeks"))
fctr <- (1.035)^(1/52 *(51 - wks_between)) 
sub$sales[ind] <- round(sub$sales[ind] * fctr, 2)

# Department trend adjustment
ind <- which(dept == 6)
wks_between <- as.integer(difftime(dt[ind], min(dt[ind]), units="weeks"))
fctr <- (0.79)^(1/52 *(51 - wks_between)) 
sub$sales[ind] <- round(sub$sales[ind] * fctr, 2)

ind <- which(dept == 5)
wks_between <- as.integer(difftime(dt[ind], min(dt[ind]), units="weeks"))
fctr <- (0.91)^(1/52 *(51 - wks_between)) 
sub$sales[ind] <- round(sub$sales[ind] * fctr, 2)

ind <- which(dept == 55)
wks_between <- as.integer(difftime(dt[ind], min(dt[ind]), units="weeks"))
fctr <- (0.83)^(1/52 *(51 - wks_between)) 
sub$sales[ind] <- round(sub$sales[ind] * fctr, 2)

ind <- which(dept == 72)
wks_between <- as.integer(difftime(dt[ind], min(dt[ind]), units="weeks"))
fctr <- (0.96)^(1/52 *(51 - wks_between)) 
sub$sales[ind] <- round(sub$sales[ind] * fctr, 2)

ind <- which(dept == 92) # Only 6.56 improvement
wks_between <- as.integer(difftime(dt[ind], min(dt[ind]), units="weeks"))
fctr <- (1.04)^(1/52 *(51 - wks_between)) 
sub$sales[ind] <- round(sub$sales[ind] * fctr, 2)
